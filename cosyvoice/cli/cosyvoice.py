...
class CosyVoice2(CosyVoice):
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1, strict_load=True, flow_ckpt=None, llm_ckpt=None):
        self.model_dir = model_dir
        self.fp16 = fp16
        if not os.path.exists(model_dir):
            model_dir = snapshot_download(model_dir)
        ...
        if not flow_ckpt:
            ...
            flow_ckpt = '{}/flow.pt'.format(model_dir)
        if not llm_ckpt:
            # Also support custom llm ckpt
...
class CosyVoice3(CosyVoice2):
    ...
    def __init__(self, model_dir, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1, strict_load=True, flow_ckpt=None, llm_ckpt=None):
        self.model_dir = model_dir
        self.fp16 = fp16
        if not os.path.exists(model_dir):
            model_dir = snapshot_download(model_dir)
        ...
        if not flow_ckpt:
            # If no flow checkpoint specified, use default
            flow_ckpt = '{}/flow.pt'.format(model_dir)
        if not llm_ckpt:
            # If no LLM checkpoint specified, use default
            llm_ckpt = '{}/llm.pt'.format(model_dir)
        self.model.load(llm_ckpt,
                        flow_ckpt,
                        '{}/hift.pt'.format(model_dir))
